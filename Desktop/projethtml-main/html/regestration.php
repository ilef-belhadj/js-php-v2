...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $message = "Format d'email invalide. Veuillez entrer une adresse email valide (exemple: nom@domaine.com)";
        $messageType = 'error';
        ...
        if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $password)) {
            ...
            $messageType = 'error';
            error_log("Registration failed - Invalid password format");
        } else {
        ...
        <div class="contact-form-container fade-in">
            ...
            <form id="registration-form" method="POST" action="">
                ...
                <input type="password" id="password" name="password"
                    ...
                    required>
                <small class="input-hint">Le mot de passe doit contenir :
                    <ul>
